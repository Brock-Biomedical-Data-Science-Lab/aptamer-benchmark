...
# Output dirs
mkdir -p rmsd_results/pos/{af3,boltz,chai,rf2na,gt} rmsd_results/neg/{af3,boltz,chai,rf2na}
...
run_rmsd_in_dir() {
  ...
  gmx rms -s md.tpr -f md_nopbc.xtc -n index_mmpbsa.ndx -tu ns -o "rmsd_${tag}.xvg" <<EOF
$fit_id
$last_id
EOF
  ...
  while IFS=',' read -r complex af3_id af3_delta chai_id chai_delta boltz_id boltz_delta rf2na_id rf2na_delta gt_delta; do
    ...
    if [[ -n "$af3_id" ]]; then
      ...
      if [[ -n "$g" ]]; then
        ...
        out_file="rmsd_results/pos/af3/${complex}_${af3_id}.csv"
        xvg_to_csv "$g/rmsd_${complex}_${af3_id}.xvg" "$out_file"
    ...
    if [[ -n "$chai_id" ]]; then
      ...
      if [[ -n "$g" ]]; then
        ...
        out_file="rmsd_results/pos/chai/${complex}_${chai_id}.csv"
        xvg_to_csv "$g/rmsd_${complex}_${chai_id}.xvg" "$out_file"
    ...
    if [[ -n "$boltz_id" ]]; then
      ...
      if [[ -n "$g" ]]; then
        ...
        out_file="rmsd_results/pos/boltz/${complex}_${boltz_id}.csv"
        xvg_to_csv "$g/rmsd_${complex}_${boltz_id}.xvg" "$out_file"
    ...
    if [[ -n "$rf2na_id" ]]; then
      ...
      if [[ -n "$g" ]]; then
        ...
        out_file="rmsd_results/pos/rf2na/${complex}_${rf2na_id}.csv"
        xvg_to_csv "$g/rmsd_${complex}_${rf2na_id}.xvg" "$out_file"
    ...
    if [[ -n "$g" ]]; then
      ...
      out_file="rmsd_results/pos/gt/${complex}.csv"
      xvg_to_csv "$g/rmsd_${complex}.xvg" "$out_file"
...
convert_neg_to_csv() {
  for model in "${NEG_MODELS[@]}"; do
    ...
    for d in "${dirs[@]}"; do
      ...
      local xvg="$d/rmsd_${lc_complex}.xvg"
      if [[ -f "$xvg" ]]; then
        local out_file="rmsd_results/neg/$model/${uc_complex}.csv"
...
convert_pos_to_csv
convert_neg_to_csv
echo "Done. CSVs under rmsd_results/."